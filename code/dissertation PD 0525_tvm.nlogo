...
to update-trustt-1
  if strategy = "random" [ act-randomly-trustt-update ]
  if strategy = "cooperate" [ cooperate-trustt-update ]
  if strategy = "defect" [ defect-trustt-update ]
  if strategy = "tit-for-tat" [ tit-for-tat-trustt-update ]
  if strategy = "unforgiving" [ unforgiving-trustt-update ]
  if strategy = "unknown" [ unknown-trustt-update ]
end

to trust-update-1
  let Sui  1
  set trustt trustt + (Sui * tvmratio)
end
...
to update-trustt-2
  ...
  foreach rank-tvmratio [ ? -> ask ? [set trustt (trustt + tvmratio)]]
end
