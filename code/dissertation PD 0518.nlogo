...
to update-trust-tvm
  if strategy = "random" [ act-randomly-trust-tvm-update ]
  if strategy = "cooperate" [ cooperate-trust-tvm-update ]
  if strategy = "defect" [ defect-trust-tvm-update ]
  if strategy = "tit-for-tat" [ tit-for-tat-trust-tvm-update ]
  if strategy = "unforgiving" [ unforgiving-trust-tvm-update ]
  if strategy = "unknown" [ unknown-trust-tvm-update ]
end

to trust-update-1
  set Sui  1
  set trust-tvm trust-tvm + (Sui * tvmratio)
end
...
to act-randomly-trust-tvm-update
  ifelse member? [strategy] of partner ["random" "cooperate" "tit-for-tat"][ trust-update-1 ][ trust-update-2 ]
end

to cooperate-trust-tvm-update
  ifelse member? [strategy] of partner ["random" "cooperate" "tit-for-tat"][ trust-update-1 ][ trust-update-2 ]
end

to defect-trust-tvm-update
  ifelse member? [strategy] of partner ["random" "cooperate" "defect"][ trust-update-1 ][ trust-update-2 ]
end

to tit-for-tat-trust-tvm-update
  ifelse member? [strategy] of partner ["random" "cooperate" "tit-for-tat"][ trust-update-1 ][ trust-update-2 ]
end

to unforgiving-trust-tvm-update
  ifelse member? [strategy] of partner ["random" "cooperate" "unforgiving" "tit-for-tat"][ trust-update-1 ][ trust-update-2 ]
end

to unknown-trust-tvm-update
  ifelse member? [strategy] of partner ["random" "cooperate" "unknown"][ trust-update-1 ][ trust-update-2 ]
end
...
;to update-trust-psm
;  if strategy = "random" [ act-randomly-trust-psm-update ]
;  if strategy = "cooperate" [ cooperate-trust-psm-update ]
;  if strategy = "defect" [ defect-trust-psm-update ]
;  if strategy = "tit-for-tat" [ tit-for-tat-trust-psm-update ]
;  if strategy = "unforgiving" [ unforgiving-trust-psm-update ]
;  if strategy = "unknown" [ unknown-trust-psm-update ]
;end

;to trust-update-3
;  set Sui = 1
;  set trust-psm trust-psm + (Sui * psmratio)
;end
;
;to trust-update-4
;  set Sui = 0
...
;
;
;to act-randomly-trust-psm-update
;  ifelse member? [strategy] of partner ["random" "cooperate" "tit-for-tat"][ trust-update-3 ][ trust-update-4 ]
;end
;
;to cooperate-trust-psm-update
;  ifelse member? [strategy] of partner ["random" "cooperate" "tit-for-tat"][ trust-update-3 ][ trust-update-4 ]
;end
;
;to defect-trust-psm-update
;  ifelse member? [strategy] of partner ["random" "cooperate" "defect"][ trust-update-3 ][ trust-update-4 ]
;end
;
;to tit-for-tat-trust-psm-update
;  ifelse member? [strategy] of partner ["random" "cooperate" "tit-for-tat"][ trust-update-3 ][ trust-update-4 ]
;end
;
;to unforgiving-trust-psm-update
;  ifelse member? [strategy] of partner? ["random" "cooperate" "unforgiving" "tit-for-tat"][ trust-update-3 ][ trust-update-4 ]
;end
;
;to unknown-trust-psm-update
;  ifelse member? [strategy] of partner ["random" "cooperate" "unknown"][ trust-update-3 ][ trust-update-4 ]
;end
;
