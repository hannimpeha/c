...
extensions [profiler gis table matrix stats time
  ;sound
]
...
to setup
  clear-all
  random-seed 1220110 ;new-seed ;1220110
  reset-ticks
  profiler:reset
  profiler:start
  set-patch-size 11
...
;------------------------------------------------------------------------------------------------------;
to total
  foreach ["data1.prn" "data2.prn" "data3.prn" "data4.prn" "data5.prn""data6.prn" "data7.prn" "data8.prn" "data9.prn" "data10.prn"][?1 ->
   set data ?1
  repeat 10 [
  setup
  build-exthouse
...
to build-house
  ...
  ;-----------------------------------------prices----------------------------------------------;
    let mean_h 5485.798
    let sd_h 18471.87
    let skewness_h 8.778329
    let kurtosis_h 126.9451
    let alpha_h 4 / (skewness_h ^ 2)
    let lambda_h sqrt alpha_h / sd_h

    let mean_j 418.6994
    let sd_j 3500.928
    let skewness_j 22.49031
    let kurtosis_j 724.0649
    let alpha_j 4 / (skewness_j ^ 2)
    let lambda_j sqrt alpha_j / sd_j

    let mean_w 38.54458
    let sd_w 396.9695
    let skewness_w 60.05504
    let kurtosis_w 3628.94
    let alpha_w 4 / (skewness_j ^ 2)
    let lambda_w sqrt alpha_w / sd_w

    set h-price precision random-normal mean_h sd_h 0
    set j-price precision random-normal mean_j sd_j 0
    set w-price precision random-normal mean_w sd_w 0
...
to set-parameters
  ...
  set p_t_previous precision random-normal mean_h sd_h 0
  set f_t_previous precision random-normal mean_h sd_h 0
  set u_t_previous precision random-normal mean_h sd_h 0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count households with [count making-whoofhouse self > 0 and [which-owner] of houses-here != self]"
